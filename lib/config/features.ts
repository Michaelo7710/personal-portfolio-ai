    ...
    case "payments": {
      ...
      if (missingEnv.length > 0) {
        return createFeatureResult({
          enabled: false,
          key: "payments",
          message: `Checkout ${provider.toUpperCase()} belum aktif. Isi ${getPaymentEnvKeys(provider).join(", ")} untuk mengaktifkan alur pembayaran end-to-end. Kalau app kamu belum butuh pembayaran, set NEXT_PUBLIC_ENABLE_PAYMENTS=false.`,
          missingEnv,
    ...
    case "ai": {
      ...
      const hasProviderKey =
        provider === "google"
          ...
          : getMissingEnv(providerEnvKeys).length === 0;

      const missingEnv = [
        ...getMissingEnv(supabasePublicEnvKeys),
        ...(hasProviderKey ? [] : providerEnvKeys),
      ];
